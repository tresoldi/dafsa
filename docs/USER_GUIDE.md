...
`benchmarks/run.py` reproduces these numbers and `tests/test_efficiency.py` guards the
...
2.0 is a clean break: it shares no implementation with 1.0, there is no compatibility
shim, and 1.0 code will not run unchanged. The reason is that weights had to change
meaning — 1.0's `lookup()` returned the summed edge counters along a path, which is
not a quantity anyone asked for — and everything else followed from giving weights an
algebra.

[**MIGRATION.md**](https://github.com/tresoldi/dafsa/blob/master/MIGRATION.md) maps the
old API onto the new one, entry by entry, and records what changed meaning rather than
merely moving.
